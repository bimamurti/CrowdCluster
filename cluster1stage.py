...
datas = getdatabyframe(df, 74)
datanframe = getdatabynframe(df, 0, 74)
datasn = np.array(datas)
...
datasnplot = reshapeperidped(np.array(datanframe))

datas=np.array(datasn)
datafit = datas[:, [5,6]]

bandwidth = estimate_bandwidth(datafit, quantile=0.2, n_samples=100)
clustersms = MeanShift(bandwidth=bandwidth, bin_seeding=True)
clust = OPTICS(min_samples=2, xi=0.005, min_cluster_size=0.005)

# Run the fit
#clust.fit(datafit)
# resK=  cluster_optics_dbscan(
#     reachability=clust.reachability_,
#     core_distances=clust.core_distances_,
#     ordering=clust.ordering_,
#     eps=2,
# )
clusters = OPTICS(metric='euclidean',min_samples=2)#KMeans(n_clusters=20)#DBSCAN(eps=0.7, min_samples=2)#OPTICS(metric='euclidean',min_samples=2) #AgglomerativeClustering(n_clusters=20, linkage='complete', metric='manhattan')
clustersK =KMeans(n_clusters=15,max_iter=200,init='k-means++')#AgglomerativeClustering(n_clusters=20)#
resK = clustersK.fit(datafit)
print(np.unique(resK.labels_))
print('len:',len(np.unique(resK.labels_)))
...
#figdirect, axdirect = plt.subplots()
...
for co in colors:
    colordict.append(co)
colordict[-1]='black'
#colordict = ['green', 'red','magenta', 'brown','grey','yellow', 'purple', 'brown', 'cyan', 'magenta', 'blue', 'orange','black']
#markerdict=['o','x','+','1','2','3','4','8','s','P','p','*','H','D','X','V','^']
markerdict=['o','x','+','1','2','3','4','s','P','*']
labels = []
labelsMarker=[]
for a in resK.labels_:
...
#     labelsK.append(index[0][0])

# lbl=0
labelsCsK=[]
for a in resK.labels_:
    labelsCsK.append(colordict[int(a)])
u=0
    ...
    if (len(indexx[0]) > 0):
       ...
       # axdir.plot(pedarr[:, [2]], pedarr[:, [3]],color=labelsCsK[idxx[0]])
        if(pedarr.size>0):
            ...
            if (pedid == 476):
                s=10
            #axdir.plot(a[5], a[6])#,color=labelsCsK[idxx[0]])
            plt.arrow(a[5], a[6], -a[5]/100, -a[6]/100, shape='full', lw=0.1, length_includes_head=False, head_width=0.009, color=labelsCsK[idxx[0]])
            labelsss=str(resK.labels_[idxx[0]])#+"/"+str(round(a[5],2))+","+str(round(a[6],2))
            axdir.annotate(labelsss, (a[5], a[6]))
    u+=1
# pedarr = np.array(datasnplot)[:,0]
# x=np.array(pedarr)[:, [5]]
# y=np.array(pedarr)[:, [6]]
# axdirect.plot(x,y,color=labelsCsK[idxx[0]])
